...
- `[1]`: rendering prompt seed — exactly 5 fields in order:
  - `zh`: 光线 (direction + color temperature + intensity) / 色调 (2–3 dominant colors) / 材质 (primary surface texture(s)) / 动态 (ambient environmental motion or "静止" if none) / 现实系数 (`realistic` / `stylized-realistic` / `heightened-reality` only — `surreal` and `fantasy` are forbidden in story_mode)
...
**Rendering prompt seed separator rule:** within `description[1]`, use hyphen `-` (NOT `:` or `：`) between each field name and its value. Field names are language-specific per the mapping above — e.g., `Reality-stylized-realistic` for `en`, `現実系数-stylized-realistic` for `ja`, never `现实系数：stylized-realistic`. This applies to all fields in the seed.
