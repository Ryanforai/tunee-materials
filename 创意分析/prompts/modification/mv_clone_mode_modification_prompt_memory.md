...
- Return JSON-parsed `history[N-1].history_mv_guide` + `style_guide` per Section 6 Style Guide and exit.
...
- If `history` has ≥ 2 entries: compare `history[0].history_mv_guide` (parsed) with `ori_mv_guide`. If they differ, return JSON-parsed `history[0].history_mv_guide` + `style_guide` per Section 6 Style Guide and exit. If they match (backend pre-populated current state into history[0]), return JSON-parsed `history[1].history_mv_guide` + `style_guide` per Section 6 Style Guide and exit.
- If `history` has exactly 1 entry AND that entry's `history_mv_guide` content differs from `ori_mv_guide`: return JSON-parsed `history[0].history_mv_guide` + `style_guide` per Section 6 Style Guide and exit. (**Comparison**: parse `history_mv_guide` as JSON, compare `md_stages`, `mv_elements.characters`, and `mv_elements.scenes` — not the raw string.)
...
2. **If Revert/Back-to-Previous detected (Sec 3.3), execute the matching revert action then exit.** (a) → full `ori_mv_guide`; (b) → `history[N-1]`; (c) → `history[0]` if ≥ 2 entries and differs from `ori_mv_guide`, else `history[1]` if pre-populated, else `history[0]` if single entry differs, else error. All with `style_guide` per Sec 6.
