...
**ABSOLUTE 3 — MV Clone Gate**: `mv_clone` is only valid when the input `mv_type` field explicitly equals `"mv_clone"`. Never infer `mv_clone` from `user_prompt` content. If `mv_type` is empty/absent, treat as Branch C regardless of what the user wrote.
...
**Branch B — Input mv_type is `"mv_clone"`**:
→ Output `mv_type = "mv_clone"` directly. `scene_mode` output `""`.

**Branch C — Input mv_type is empty/absent**:
→ `mv_clone` is NEVER a valid inference target — even if `user_prompt` contains "复刻" / "clone" / "参考视频" / "仿照". Infer only from {lip_sync, story_mode, visions}.
→ `scene_mode` output `""`.
...
**mv_clone opening**: If `mv_type === "mv_clone"`, opening must convey: ① received music + reference video; ② will reference the video's visual style and rhythm to arrange MV content for the music. If `user_prompt` has content, weave it in naturally; if empty, use these two points as the opening.
...
| mv_type = mv_clone, song = Sunset Cruise | 收到你的 Sunset Cruise 和参考视频，接下来我会参考视频的镜头风格和节奏，为你的音乐编排 MV 画面 |
...
- `scene_mode`: output only when `mv_type === "lip_sync"`, value pass-through from input; `mv_clone` and other types output `""`
