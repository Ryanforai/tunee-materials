...
| 焦段 | 用途 |
|------|------|
| 35–50mm | 平静客观，世界规则建立段，空间与人物并重 |
| 85mm | 唯美高级感，情绪人像，仪式感动作 |
| 135mm | 疏离感，人物在宏大幻想空间中的孤独 |
| 200mm+ | 极度隔离，压缩比例失调，偷窥感 |
| Macro / 100mm | 材质细节特写，皮肤/水晶/液态金属边界 |
| 超广角（16-24mm） | 极端空间展示，宏大世界规则全景 |
...
**幻想光源（纯幻想专属）：**
- `Sourceless ambient light emanating from the ground itself`
- `Multiple moons casting overlapping soft shadows`
- `Light that bends around corners in non-Euclidean paths`
- `Objects themselves glowing with internal cold luminescence`
...
**重力系统：**
- 正常：`standard gravity, objects fall downward`
- 反转：`inverted gravity, objects fall upward, characters standing on ceiling`
- 无重力：`zero gravity, objects and figures float freely`
- 局部：`gravity applies only within lit zones, stepping into shadow means floating`

**空间系统：**
- 有限：`confined architectural space with impossible angles`
- 无限延伸：`infinite mirrored corridor / horizon that never arrives`
- 多空间并存：`two spaces occupy the same location simultaneously`

**材质系统：**
- `crystalline surfaces that refract light into impossible spectra`
- `liquid metal ground that reflects but also absorbs figures`
- `gaseous architecture — walls of compressed colored smoke`
- `objects transitioning between solid and liquid state without heat`
...
**比例失调：**
- `human figure dwarfed by architecture ten times their height`
- `ordinary objects scaled to building size`
- `reverse scale: enormous person in miniature environment`

**物理违背：**
- `explosion frozen mid-blast, glass shards suspended mid-air`
- `multiple time speeds: foreground in slow motion, background normal`
- `gravity reversed for one object only while everything else is normal`

**非人感：**
- `skin becoming translucent, veins and bones visible beneath`
- `fingers dissolving into crystalline particles at the tips`
- `hair behaving like liquid, moving against wind direction`
...
超然：eyes half-closed, breathing slowed, body perfectly still
```
