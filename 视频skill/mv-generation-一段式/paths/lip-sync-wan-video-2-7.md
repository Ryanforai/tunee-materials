...
有效风格值存在时判定，决定 frame_prompt ④⑦ 写法。**两族同时命中以画风系优先；无法判定时兜底摄影系。**

| 渲染族 | 判定关键词 | frame_prompt ④⑦ 行为 |
|--------|-----------|----------------------|
| **摄影系 `photographic`** | 写实 / realistic / cinematic / 电影 / 胶片 / film / vintage / 复古 / Y2K / noir / 黑色电影 / 赛博朋克 / cyberpunk / documentary / 纪录 / CG写实 / 真人 / live-action / 精致写实 / 高级质感 | ④ 焦段 + 虚实完整；⑦ 相机 + 胶片完整 |
| **画风系 `stylized`** | 动漫 / anime / 赛璐璞 / cel / 水墨 / ink wash / 水彩 / watercolor / 油画 / oil painting / 3D渲染 / 3D render / toon / 像素 / pixel / 浮世绘 / ukiyo-e / 国画 / 漫画 / comic / manga / 剪纸 / paper cut / 插画 / illustration / flat design / 低多边形 / low poly / voxel | ④ 仅焦段数字（透视）；⑦ 跳过 |
...
全片 **canonical stage plan**。Markdown 表格，至少包含列：`时间段` / `音乐结构` / `歌词` / `画面描述` / `场景` / `角色`
...
- 只有 `mv_elements.characters[] / scenes[]` 中显式存在的对象才能进入资源表；`md_stages`、`lyrics`、`visual_brief`、case 等描述性文本不能作为创建新资源对象的依据
...
### Stage 时间段原则（wan_video_2_7）

Stage 层保持输入的时间切分不变；Shot 层根据合法时长范围 **[2, 15]s** 适配：

- stage 时长 > 15s → 拆分为多个时长 ∈ [2, 15] 的 shot
- stage 时长 < 2s → 吸收进相邻 shot 作为内部 beat
...
| scene_mode | 约束 |
|------------|------|
| `one_take` | `audio_duration` ∈ [2, 15]；shots 数组长度 = 1 |
| `multiple_scenes` | 每 shot 时长 ∈ [2, 15]；每 shot ≤ 1 主运镜 |

- `one_take` 且 `audio_duration` ∉ [2, 15] → error JSON
- `multiple_scenes` 中存在时长 < 2s 或 > 15s 的 shot → error JSON

> §2 中 stage < 2s 时已吸收进相邻 shot，合并后的 shot 时长须满足 ∈ [2, 15]。stage < 2s 本身不单独成 shot。
...
- 默认优先 `1 stage → 1 shot`
- 若某个 stage 时长超出 [2, 15] 或在镜头上不够独立成立，可拆分或与相邻兼容 stage 合并
...
□ 每 shot 时长 ∈ [2, 15]
□ one_take → audio_duration ∈ [2, 15]，shots 长度 = 1
...
□ scene_name 最多 1 个场景
□ 有 stage 时长 < 2s 时，已吸收进相邻 shot；合并后 shot 时长 ∈ [2, 15]
```
