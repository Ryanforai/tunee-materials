...
> 真实场景，情绪/概念/内心世界的视觉外化。输入已给出每段画面意图，本文件负责把这些意图拍得更精致、更有层次。
> **执行时同时加载 refs/visions-realistic-ref.md（词库 + Case）。**
...
1. 情绪优先于逻辑，但仍服从输入的 stage 顺序
2. 人物是情绪容器，不必做复杂事件，但必须有明确身体状态
3. 重复不是照抄，同一意象每次都要换语境、角度、光线或距离
4. 副歌段把视觉密度推到最强，但保留可拍的现实质感
...
优先类型：逆光剪影 + 大面积留白 / 正中央孤立人物 + 对称空间 / 强单侧光 + 半明半暗的脸 / 近景人像 + 前景遮挡
...
词库见 refs/visions-realistic-ref.md
...
**精致写实专属禁止：** `handheld shake` / `gritty` / `raw` / `unpolished` / `neon`

**纪录写实专属禁止：** `smooth gimbal` / `perfectly centered` / `rim light` / `god ray`
