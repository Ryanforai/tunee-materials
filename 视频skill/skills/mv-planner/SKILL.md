...
references/
  ...
  creative/
    ...
    lip-sync-kling-avatar.md          ← 口播·kling_avatar
    lip-sync-wan-video.md             ← 口播·wan_video
```
...
- `effective_max_duration = min(video_model.max_duration, 12)`；未提供时默认 `12`
- `min_duration` 从 `video_model` 读取，默认 `3`
- 正常 stage → 1 stage = 1 SHOT
- 超长 stage（> effective_max_duration）→ 拆成多个 SHOT，每个时长合法
...
Q3: visual_brief 存在轻度物理违背？ → 是 → visions + surreal → 否 → visions + realistic
```
