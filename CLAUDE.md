...
**每次编写或修改需求文档后，必须运行 `/prd-audit` 或说「审一下」来检查文档质量。**

项目已安装 `prd-audit` skill（位于 `.claude/skills/prd-audit/`），覆盖七个维度：背景与目标、功能范围与边界、正常路径、异常与边界情况、跨需求与多端横切影响、外部依赖与 API 效果验证、埋点与度量。

写需求文档 ≠ 完成。写完 → 审计 → 补漏 → 再评审，是标准流程。
